...
def remove_chains(fixer, chains_to_keep):
    """Uses fixer.remove_chains to remove chains not present in
    chain_to_keep. Keeps the first occurence of each chain.

    Args:
        fixer (pdbfixer.pdbfixer.PDBFixer): fixer object for fixing pdbs.
        chains_to_keep (list[str]): list of chain ids to keep.
    Returns:
        None
    """
    seen = set()
...
def correct_oxt(pdb, output_filename=None):
    ...
    Args:
        ...
        output_filename(str|Path, optional): if not None,
        path to write out. Defaults to None.
    Returns:
        None
    """
    ...
    for chain in fixer.topology.chains():
        ...
        for atom in chain.atoms():
            if (atom.residue.id != str(n) and atom.residue.id != str(0)
               and atom.name == 'OXT'):
                atom.name = 'O'
                num_oxt_fixed += 1
...
def download_pdb(pdb_id, protein_name='', output_path=None, atoms_only=False):
    ...
    Args:
        ...
        output_path (str|Path, optional): save directory. Defaults to None.
        atoms_only (bool, optional): write only atoms? Defaults to False.
    """
...
def extract_fv_from_pdb(pdb, output_pdb=None, scheme='kabat'):
    ...
    for chain in struct.get_chains():
        ...
        # iterate through the PDB chain object and add residues of interest.
        residue_num = 1
        num = 0
...
def mutate_protein(pdb, mutations, output_path=None, output_filename=None,
                   keep_ids=True, transform_mutants=True, pH=7.0):
    """Uses PDBFixer to mutate antibody, save the file

    Args:
        pdb (str|path): path to pdb file
        mutations (dict): dict with keys for chain and values of list of
                          mutations.
        output_filename (str, optional): name of file. Defaults to None.
        output_path (str|Path, optional):output path. Defaults to None.
        keep_ids (bool, optional): Whether to keep original chain ids
                                   and numbering. Defaults to True.
        transform_mutants (bool, optional): If True, transform 1 letter
                                            code to 3 letter code.
                                            Defaults to True.
    ...
    PDBFile.writeFile(fixer.topology, fixer.positions,
                      open(str(output), 'w'), keepIds=keep_ids)
    return output
...
def transform_mutant_tuple(mutant):
    ...
    Returns:
        str:
    """
    aa1 = seq3(mutant[0]).upper()
    pos = int(mutant[1])
    aa2 = seq3(mutant[2]).upper()
...
def generate_mutations_dict(row, lc_length, hc_length):
    mutations = {'L': [mutation for mutation in row['Vl mutations']
                       if mutation[1] <= lc_length],
                 'H': [mutation for mutation in row['Vh mutations']
                       if mutation[1] <= hc_length]
                 }
    return mutations
