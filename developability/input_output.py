...
def read_xyzr(xyzr_file): 
    ...
    """
    names  = ['X', 'Y', 'Z', 'Radius']
    df = pd.read_csv(xyzr_file, delim_whitespace=True, header = None,names = names)
    return df


def extract_xyz_atoms_from_off(off_file): 
    """ Extracts the xyz coordinates and associated atoms from the given off surface file.
    Args:
