...
MULTIVIEW_PATH = (Path('/Users/ChristopherRivera/bioprograms/') /
                  'APBS-3.4.1.Darwin/share/apbs/tools/bin')


def generate_prm_file(xyzr_filename='', output_path=None, options=None):
    """ Generates a prm (parameter) file for NanoShaper.
    Args:
    ...
    # there are many options in nanoshaper. Using these for now.
    # These are the only options one can update for now,
    # without updating the template.prm.
    ...
    # for now keeping the template in same dir
    template_path = files('developability')
...
def run_nanoshaper(prm_file, save_log=True, working_directory=None):
    ...
    Args:
        ...
        working_directory(None|Path): The working directory to
                                      run command from.
...
def sum_atom_potential(potential_coordinates_file=None,
                       atom_coordinates_file=None):
    """ Sums up the potentials per atom.
    Args:
        potential_coordinates_file (str|Path): path to potential coordinates
                                               dx file.
        atom_coordinates_file (str|Path): path to file with atom
                                          and coordinates.
    Returns:
    ...
    names_map = {'len': 'number', 'sum': 'total_pot',
                 'mean': 'mean_pot', 'atom': 'Atom_number'}

    return res.rename(names_map, axis=1)


def map_potential_to_atoms(pqr_file, potential_coordinates_file=None,
                           atom_coordinates_file=None, output_dir=None):
    """ Map Electric Surface Potential to atoms on PQR file.
        Saves output as csv and returns df.
    Args:
        pqr(Path/str): path to pqr file
        potential_coordinates_file (Path/str): path to potential
                                               coordinates dx file.
    ...
    atom_pot_df = sum_atom_potential(
        potential_coordinates_file, atom_coordinates_file)
...
def sum_potential_over_residues(atom_pot_df, output_dir):
    """ Given a dataframe with the potentials summed per atoms,
            sums up over residues.
    Args:
        atom_pot_df(pd.DataFrame): dataframe with potential
        and atoms.
...
class SurfacePotential(object):
    ...
    def __init__(self,
        ...
        # these are the files that will be generated by the class
        self.surface_file = output_dir/'triangulatedSurf.off'
        self.coordinates_file = output_dir/'triangulatedSurf.csv'
        self.atom_coordinates_file = output_dir/'triangulatedSurf.atom.csv'
        self.potential_coordinates_file = (output_dir /
                                           'potential_coordinates.csv')
        self.atom_potential_file = output_dir/'atom_potential.csv'
        self.residue_potential_file = output_dir/'residue_potential.csv'

    def _generate_input_files_(self):
        """Generates the input files such as the prm file and xyzr file
    ...
    def calculate_surface_potential(self, verbose=True):
        ...
        atom_pot_df = map_potential_to_atoms(self.input_pqr,
                                             self.potential_coordinates_file,
                                             self.atom_coordinates_file,
                                             self.output_dir
                                             )

        print('Finished mapping potentials to atoms.')
        print('Now aggregating potentials over residues.\n')
        _ = sum_potential_over_residues(atom_pot_df, self.output_dir)
...
if __name__ == '__main__':
    output = (Path().home() /
              '/Projects/developability/developability/data/pdbs/test')
