...
def run_pdb2pqr(input_pdb, output_path, options=None, save_log=True): 
    """Runs PDB2PQR from commandline to create pqr file from pdb for running APBS
    Parameters:  
...
def __update_elecs__(input_, options): 
    """helper function to update elecs of inputgen.Input object for controlling APBS calculations.
    Args: 
...
class APBS(object): 
    ...
    def __init__(self, 
        ...
        Args:  
            ...
            pdb2pqr_options(dict|None): dict with options for controlling pb2pqr
            apbs_options(dict|None): dict for controlling input parameter file 
    ...
    def calculate_potential(self, verbose=True):
        ...
        Returns: 
            self.pqr_file(Path): patht to the pqr file
        ...
        print(f'Generating APBS ini file.\n')
        _ = generate_apbs_ini(self.pqr_file, self.ini_file, options=self.apbs_options)
...
if __name__=='__main__':
    pass
    """
    pdbs_path = Path(data.pdbs.__path__[0]) 
    input_pdb = pdbs_path /'abagovomab_forChris.energy_minimized.pdb'
    
    output_path = pdbs_path/'test'
    apbs = APBS(input_pdb, output_path)
    apbs.calculate_potential()
   """