...
class TriangulatedSurface:
    ...
    Attributes:
        ...
        vertices (pandas.DataFrame): A DataFrame containing the x, y,
                                     and z coordinates of each vertex.
        faces (pandas.DataFrame): A DataFrame containing the indices of
                                  the vertices that make up each face.
        face_areas (numpy.ndarray): An array containing the area of each face.
        total_area (float): The total area of the surface.

    Methods:
        get_vertices_for_face(face): Returns the indices of the vertices
                                     that make up a given face.
        get_vertex_cordinates(vertex): Returns the x, y, and z coordinates
                                       of a given vertex.
    ...
    def __init__(self, fname):
        ...
        self.num_vertices, self.num_faces, self.num_edges = list(
            map(int, self.text[1].split()))
        ...
        face_dict, vert_dict = self.__generate_face_vertices_dicts__
        self.face_dict, self.vertices_dict = face_dict, vert_dict
    ...
    def _parse_lines_(self, start, end, num_type=float):
        ...
        """
        lines = [list(map(num_type, line.split(' ')))
                 for line in self.text[start:end]]
        return lines
    ...
    def _parse_vertices_(self):
        ...
        Returns:
            pandas.DataFrame: A DataFrame containing the x, y,
                              and z coordinates of each vertex.
        """
    ...
    def _parse_faces_(self):
        ...
        Returns:
            pandas.DataFrame: DataFrame containing the indices
                              of the vertices that make up each face.
        """
        faces = self._parse_lines_(
            self.num_vertices+2, self.num_vertices+self.num_faces+2, int)
    ...
    def get_vertices_for_face(self, face):
        ...
        Returns:
            tuple: A tuple containing the indices of the vertices
    ...
    def get_vertex_cordinates(self, vertex):
        ...
        Returns:
            numpy.ndarray: An array containing the x, y, and z
    ...
    def __generate_face_vertices_dicts__(self):
        """ Generates a dictionary of faces and vertices
            and a dictionary of vertices and faces
        """
        face_dict = {}
        verticies_dict = {}
    ...
    def __share_two_vertices__(self, f1, f2):
        """Determines if two surface faces share at least two vertices
           (ie a side)"""
        intersection = set(self.face_dict[f1]).intersection(self.face_dict[f2])
    ...
    def _face_areas_(self):
        ...
        return pd.DataFrame(np.linalg.norm(np.cross(s1, s2), axis=1) / 2,
                            columns=['area'])
...
def aggregate_feature_over_vertices(faces_dict, features, columns,
                                    aggfunc=np.mean):
    ...
    for face, vertices in faces_dict.items():
        agg.setdefault(face, [])
        for col in columns:
            agg[face].append(aggfunc([feature_dicts[col][v]
                             for v in vertices]))
...
def break_edges(g, attribute):
    """Break edges in a graph based on nodes not having same value
       for attribute.
    Args:
...
def aggregate_features_over_components(g, components, attributes,
                                       aggfunc=np.sum):
    ...
    for i, component in enumerate(components):
        ...
        for attribute in attributes:
            agg[i][attribute] = aggfunc(
                [g.nodes[n][attribute] for n in component])
...
def compute_surface_potential_patches(path, share_two=True,
    ...
    aggregated_potential = aggregate_feature_over_vertices(
        ts.face_dict, potential_df, ['potential', 'charge_sign'])
    add_data_from_dataframe(g, aggregated_potential, [
                            'charge_sign', 'potential'])
    ...
    df = aggregate_features_over_components(g, components,
                                            attributes=['area',
                                                        'charge_sign',
                                                        'potential'],
                                            aggfunc=np.sum)
