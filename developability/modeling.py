...
import numpy as np
from sklearn.metrics import (PredictionErrorDisplay, PrecisionRecallDisplay, RocCurveDisplay, average_precision_score,
                             mean_squared_error, r2_score, roc_auc_score, accuracy_score, f1_score)
...
def plot_regression_performance_from_predictions( y_true, y_pred, model_name= None, dataset='validation', figsize = (8, 4), set_lim=False):
...
def extract_train_test_cv_scores(grid, scores=None, decimals=3, n_total = 50):
    ...
    for score in scores: 
        ...
        scores_dict[f'train_cv_{name}']= np.abs(scores_df[f'mean_train_{score}'].values[0])
        scores_dict[f'train_cv_{name}_sem']= np.round(scores_df[f'std_train_{score}'].values[0]/np.sqrt(n_total), decimals)
        scores_dict[f'test_cv_{name}']= np.abs(scores_df[f'mean_test_{score}'].values[0])
        scores_dict[f'test_cv_{name}_sem']= np.round(scores_df[f'std_test_{score}'].values[0]/np.sqrt(n_total), decimals)
...
classifier_params = {'rf': {'rf__n_estimators': [50, 100, 200], 'rf__max_depth': [3, 5, 7, 9], 'rf__max_features': [ 'sqrt', 'log2']}, 
                    'logistic': {'logistic__penalty': ['l1', 'l2', 'elasticnet'], 'logistic__C': np.logspace(-4, 4, 9), 'logistic__l1_ratio': np.linspace(0, 1, 10)},
                    'et': {'et__n_estimators': [50, 100, 200], 'et__max_depth': [3, 5, 7, 9], 'et__max_features': [ 'sqrt', 'log2']}
                    }

regressor_params = {'rf': {'rf__n_estimators': [50, 100, 200], 'rf__max_depth': [3, 5, 7, 9], 'rf__max_features': [ 'sqrt', 'log2']},
                    'linear': {'linear__fit_intercept': [True]},
                    'ridge': {'ridge__alpha': np.logspace(-4, 4, 9)},
                    'lasso': {'lasso__alpha': np.logspace(-4, 4, 9)},
                    'elasticnet': {'elasticnet__alpha': np.logspace(-4, 4, 9), 'elasticnet__l1_ratio': np.linspace(0, 1, 10)},
                    'et': {'et__n_estimators': [50, 100, 200], 'et__max_depth': [3, 5, 7, 9], 'et__max_features': [ 'sqrt', 'log2']}
                    }
...
class MLFlowExperiment:
    ...
    def __init__(self, data_path, target, experiment_name, regression=True, n_splits=5, n_repeats=10, 
                 random_state=42, scoring=None, scaler=None, models=None, tracking_uri=None, feature_sets=None, 
                 fit_single_features=False): 
        ...
        if not feature_sets:
            feature_sets = dict(
                pos_cols = [col for col in self.data.columns if 'pos' in col and ('1' in col or '2' in col or '3' in col or '4' in col)],
                net_cols = [col for col in self.data.columns if 'net' in col and ('1' in col or '2' in col or '3' in col or '4' in col)]
            )
        self.feature_sets = feature_sets

        self.fit_single_features = fit_single_features
        if self.fit_single_features:
            self.single_features = {col:[col] for col in self.data.columns if col != self.target}
    ...
    def __load_data__(self):
        ...
        elif self.data_path.name.endswith('.parquet') or self.data_path.name.endswith('.pq'):
            return pd.read_parquet(self.data_path)


    def get_cv_splitter(self, X,y):
        if self.regression: 
            return split_continous(X, y, n_splits=self.n_splits, n_repeats = self.n_repeats, 
                                   random_state = self.random_state) 
        else:
            return RepeatedStratifiedKFold(n_splits=self.n_splits, n_repeats=self.n_repeats, 
    ...
    def gridsearch(self, model_name, features, single_feature=False):
        ...
        grid = GridSearchCV(pipes, params, scoring=self.scoring, cv=cv, 
                            n_jobs=n_jobs_for_search , refit=self.scoring[0], verbose=1, return_train_score=True)
        ...
        with mlflow.start_run():
            ...
            cv = self.get_cv_splitter(X,y)
            mean_predictions, predictions_df = get_average_predictions(X,y, grid.best_estimator_, cv = cv, regression =self.regression)
            ...
            scores = extract_train_test_cv_scores(grid, scores=self.scoring, n_total=self.n_splits*self.n_repeats)
            ...
            mlflow.sklearn.log_model(grid.best_estimator_, model_name , signature=signature)
